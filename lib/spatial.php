...
class Rectangle
{
	...
	//------------------------------------------------------------------------------------
	// Assumes rectangle is not rotated or anything fancy, for which see 	
	...
	//------------------------------------------------------------------------------------
	// Return overlap of this rectangle with $otherRect. If they don't intersect, return null
	function getOverlap($otherRect, $error = 1)
	{
		$overlap = null;
		...
		if ($this->intersectsRect($otherRect))
		{
			...
			// sanity check
			if ($overlap->w < $error  || $overlap->h < $error )
			{
				$overlap = null;
			}
	...
	//------------------------------------------------------------------------------------
	// Merge rect with another 
	function merge($otherRect)
	{
		$r1_topleft = $this->getTopLeft();
		$r2_topleft = $otherRect->getTopLeft();
		
		$pt0 = new Point(
			min($r1_topleft->x, $r2_topleft->x),
			min($r1_topleft->y, $r2_topleft->y)
		);
		...
		$pt1 = new Point(
			max($r1_bottomRight->x, $r2_bottomRight->x),
			max($r1_bottomRight->y, $r2_bottomRight->y)
		);
	...
	function toSvg()
	{
		$svg = '<rect id="' . $this->id . '" vector-effect="non-scaling-stroke" x="' . $this->x . '" y="' . $this->y . '" width="' . $this->w . '" height="' . $this->h . '"
  style="stroke:black;fill:rgb(' . rand(0,256) . ',' . rand(0,256) . ',' . rand(0,256) . ');fill-opacity:0.1;"/>'; 
  
  		if ($this->name != '')
  		{
  			$svg .= '<text font-size="1em" x="' . ($this->x + 10) . '" y="' . ($this->y + $this->h/2) . '">' . $this->name . '</text>';
  		}
 		return $svg;
	}
	
	//------------------------------------------------------------------------------------
	// Rectangle to array of values
	function toArray()
	{
		$output = array(
			'x' => $this->x,
...
class BBox
{
	...
	//------------------------------------------------------------------------------------
	// QuadPoints for use in Adobe PDF highlighting
	// It appears as if the order is: TopLeft, TopRight, BottomLeft, BottomRight.
	// https://stackoverflow.com/q/9855814/9684
	// For this function we assume (0,0) is bottom right
	function toQuadPoints($w = 1, $h = 1)
	{
		$topleft = new stdclass;
		$topleft->x = $this->minx/$w;
		$topleft->y = $this->maxy/$h; // not maxy
		
		$topright = new stdclass;
		$topright->x = $this->maxx/$w;
		$topright->y = $this->maxy/$h; // not maxy
		
		$bottomright = new stdclass;
		$bottomright->x = $this->maxx/$w;
		$bottomright->y = $this->miny/$h; // not miny

		$bottomleft = new stdclass;
		$bottomleft->x = $this->minx/$w;
		$bottomleft->y = $this->miny/$h; // not miny
	...
	//------------------------------------------------------------------------------------
	// Bounding box to simple array of values
	function toArray()
	{
		$output = array(
			$this->minx,
